...
<h4 class="start-page-title">{!! apply_filters('the_title', $post_title) !!}</h4>
@endif
        ...
        <div class="grid grid--columns" data-equal-container>
            ...
            @if (count($posts) > 0)
                @foreach ($posts as $post)
                    ...
                    <div class="u-flex grid-xs-12 grid-sm-6 grid-md-4 grid-lg-3">
                            ...
                            <div class="card-body">
                                ...
                                @if (in_array('excerpt', $posts_fields))
                                    @if ($posts_data_source === 'input')
                                        {!! $post->post_content !!}
                                    @else
                                        <p>{!! isset(get_extended($post->post_content)['main']) ? apply_filters('the_excerpt', substr(wp_trim_words(wp_strip_all_tags(get_extended($post->post_content)['main']), 30, null), 0, 100) . '...') : '' !!}</p>
                                    @endif
                                @endif
